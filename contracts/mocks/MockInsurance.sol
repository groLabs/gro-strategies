...
contract MockInsurance is IInsurance {
    ...
    function getVaultDeltaForDeposit(uint256 amount) external view override returns (uint256[3] memory, uint256) {
        amount;
        if (vaultDeltaIndex == 3) {
            return (vaultDeltaOrder, 3);
        } else {
    ...
    function setUnderlyingTokenPercents(uint256[3] calldata percents) external override {
