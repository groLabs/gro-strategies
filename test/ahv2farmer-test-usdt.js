...
contract('Alpha homora test', function (accounts) {
  ...
  beforeEach(async function () {
    ...
    usdtAdaptor = await VaultAdaptor.new(tokens.usdt.address, bouncer, { from: governance });
  ...
  describe('Assets interactions', function () {
    ...
    // should be able to estimate totalAsset changes
  ...
  describe("Utility", function () {
    ...
    it('Should revert if a an AMM check fails', async () => {
        ...
        await expect(usdtAdaptor.strategyHarvest(0, amount_norm_usdt, amount_norm_weth, {from: governance})).to.eventually.be.rejected;
        ...
        // force close the position
        return expect(primaryStrategy.forceClose(position, amount_norm_usdt, amount_norm_weth, {from: governance})).to.eventually.be.rejected;
    })
    ...
    it('Should be possible for the amm check to use multiple tokens', async () => {
        ...
        await setBalance('usdt', investor1, amount);
        // get spotPrice
        const change_want = await sushiSwapRouter.methods.getAmountsOut(toBN(1E18), [avax.address, usdt.address]).call();
        const change_joe = await sushiSwapRouter.methods.getAmountsOut(toBN(1E18), [sushi.address, usdt.address]).call();
        ...
        await usdtAdaptor.deposit(amount_norm_usdt, {from: investor1})
        await usdtAdaptor.strategyHarvest(0, [avax.address, sushi.address], [minWantAvax, minWantJoe], {from: governance})
        await expect(primaryStrategy.activePosition()).to.eventually.be.a.bignumber.gt(toBN(0));
        // force close the position
        const position = await primaryStrategy.activePosition();
        await expect(primaryStrategy.forceClose(position, [usdt.address], [toBN(1E18)], {from: governance})).to.eventually.be.rejected;
    ...
    it('Harvest trigger', async () => {
        ...
        await setBalance('usdt', investor1, amount);
        await usdtAdaptor.deposit(amount_norm, {from: investor1})
        ...
        await setBalance('usdt', investor1, amount);
        await usdtAdaptor.deposit(sec_amount_norm, {from: investor1})
        await expect(primaryStrategy.harvestTrigger('0')).to.eventually.be.equal(false);
