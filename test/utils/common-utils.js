...
function getBalanceOfSlotVyper(mappingSlot, address) {
    return ethers.utils.hexStripZeros(ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(['uint256', 'address'], [mappingSlot, address])));
}

async function setBalance(tokenSymbol, to, amount, lang=0) {
        const amountToMint = web3.utils.padLeft(ethers.utils.parseUnits(amount, tokens[tokenSymbol].decimals ).toHexString(), 64);
        let slot;
        if (lang == 0) {
            slot = getBalanceOfSlotSolidity(tokens[tokenSymbol].mappingSlot, to);
        } else {
            slot = getBalanceOfSlotVyper(tokens[tokenSymbol].mappingSlot, to);
        }
